...
function updateBadge(tabId, count){
  ...
  try{
    const act = (typeof browser !== "undefined" && browser.action) ? browser.action : (typeof chrome !== "undefined" ? chrome.action : null);
    if (!act) return;
    act.setBadgeText({tabId, text: count ? String(count) : ""});
    act.setBadgeBackgroundColor({tabId, color: "#4f8cff"});
  }catch{}
}
